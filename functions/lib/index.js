...
exports.deleteCropListing = exports.markCropAsSold = exports.updateMarketRates = exports.postCropForSale = exports.getAIResponseForQuery = exports.sendDailyWeatherAlerts = void 0;
// functions/index.ts
...
const WEATHER_API_KEY = (_a = functions.config().weather) === null || _a === void 0 ? void 0 : _a.key;
const AI_API_KEY = (_b = functions.config().ai) === null || _b === void 0 ? void 0 : _b.key;
...
exports.getAIResponseForQuery = functions.firestore
    ...
    const prompt = {
        ...
        messages: [
            {
                role: "system",
                content: "You are Verda, a helpful farming assistant for farmers in Pakistan. You must answer in both simple English and Urdu (using Roman Urdu/Urdu script). Provide clear, actionable advice.",
            },
    ...
    try {
        ...
        const aiRes = await axios_1.default.post("https://openrouter.ai/api/v1/chat/completions", prompt, {
            headers: {
                "Authorization": `Bearer ${AI_API_KEY}`,
                "Content-Type": "application/json",
                "HTTP-Referer": "https://farming-app.com",
        ...
        console.error("Error getting AI response:", ((_a = error.response) === null || _a === void 0 ? void 0 : _a.data) || error.message);
...
exports.postCropForSale = functions.https.onCall(async (data, context) => {
    ...
    if (!cropName || !quantity || !price || !unit) {
        throw new functions.https.HttpsError("invalid-argument", "Missing required fields: cropName, quantity, price, unit");
    }
    // Validate data types
    if (typeof quantity !== "number" || typeof price !== "number") {
        throw new functions.https.HttpsError("invalid-argument", "Quantity and price must be numbers");
    }
    if (quantity <= 0 || price <= 0) {
        throw new functions.https.HttpsError("invalid-argument", "Quantity and price must be positive numbers");
    }
    ...
    try {
        ...
        const cropRef = await admin.firestore().collection("crops_for_sale").add({
            sellerId: uid,
            sellerName: (userData === null || userData === void 0 ? void 0 : userData.name) || "Anonymous",
            sellerPhone: (userData === null || userData === void 0 ? void 0 : userData.phone) || "",
            sellerLocation: location || (userData === null || userData === void 0 ? void 0 : userData.location) || "",
            cropName: cropName,
            urduName: urduName || "",
            quantity: quantity,
            price: price,
        ...
        throw new functions.https.HttpsError("internal", `An error occurred while posting your crop: ${error.message}`);
    }
});
// --- FUNCTION 4: Update Market Rates (Admin) ---
exports.updateMarketRates = functions.https.onCall(async (data, context) => {
    // Check authentication
    if (!context.auth) {
    ...
    if (!Array.isArray(rates) || rates.length === 0) {
        throw new functions.https.HttpsError("invalid-argument", "Rates must be a non-empty array");
    }
    ...
    rates.forEach((rate) => {
        ...
        if (!rate.name || !rate.price || !rate.unit) {
            throw new functions.https.HttpsError("invalid-argument", "Each rate must have name, price, and unit");
        }
    ...
    try {
        ...
        throw new functions.https.HttpsError("internal", `Error updating rates: ${error.message}`);
    }
});
// --- FUNCTION 5: Mark Crop as Sold ---
exports.markCropAsSold = functions.https.onCall(async (data, context) => {
    var _a;
    if (!context.auth) {
        throw new functions.https.HttpsError("unauthenticated", "You must be logged in");
    }
    const { cropId } = data;
    if (!cropId) {
        throw new functions.https.HttpsError("invalid-argument", "cropId is required");
    }
    try {
        const cropRef = admin.firestore().collection("crops_for_sale").doc(cropId);
        const cropDoc = await cropRef.get();
        if (!cropDoc.exists) {
            throw new functions.https.HttpsError("not-found", "Crop listing not found");
        }
        // Verify the user owns this listing
        if (((_a = cropDoc.data()) === null || _a === void 0 ? void 0 : _a.sellerId) !== context.auth.uid) {
        ...
        throw new functions.https.HttpsError("internal", error.message || "An error occurred");
    }
});
// --- FUNCTION 6: Delete Crop Listing ---
exports.deleteCropListing = functions.https.onCall(async (data, context) => {
    var _a;
    if (!context.auth) {
        throw new functions.https.HttpsError("unauthenticated", "You must be logged in");
    }
